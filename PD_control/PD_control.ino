...
// Servo range ------------------------------------------------------------
#define _DUTY_MIN 1820 //[3028] 서보 각도 최소값
#define _DUTY_NEU 1650 //[3038] 레일 수평 서보 펄스폭
#define _DUTY_MAX 1330 //[3031] 서보 최대값

// Servo speed control------------------------------------------------------------
#define _SERVO_ANGLE 60 //[3030] servo angle limit 실제 서보의 동작크기
                        //[3023] 서보모터의 작동 범위(단위 : degree)
#define _SERVO_SPEED 2000 //[3040] 서보의 각속도(초당 각도 변화량)
...
// PID parameters------------------------------------------------------------
#define _KP 1.4    //[3039] 비례 제어의 상수 값
//#define _KP 0    //[3039] 비례 제어의 상수 값
#define _KD 58    //[3039] 비례 제어의 상수 값
...
void loop() {
  ...
  control = pterm + dterm ;
//    duty_curr = (duty_target + control)*6.4; //[3034] --> 중립값에서 control만큼 움직임.  
 ...
 // update error_prev
  ...
  duty_curr = (duty_target+ control)*1.15; //[3034] --> 중립값에서 control만큼 움직임.
// duty_target 값을 [_DUTY_MIN, _DUTY_MAX] 범위 내로 유지

//    거리센서와 스파이크 보정하는거 추가해야 함. (나중에 시연에서 튕기면 안됨.) 중심도 잘 잡아야 함.
